...
pert_resp = get_formant_pert_response(trial_pert_types,parsed_formant_in,parsed_formant_out,good_trials,parsed_frame_taxis,[],pertclass_formant_types);

if (yes_overwrite >= 3) || ~exist('baseline4comp.mat','file')
  formant_make_baseline4comp(pert_resp,colors,parse_fig_pos);
end
load('baseline4comp');

pert_resp.baseline = baseline;
pert_resp.baseline_type = baseline_type;
npert_types = pert_resp.npert_types;
for ipert_type = 1:(npert_types+1)
 baseline_repmat = repmat(pert_resp.baseline,[pert_resp.n_good_trials(ipert_type) 1]);

  pert_resp.formant_in.dat{ipert_type} = pert_resp.formant_in.dat{ipert_type} - baseline_repmat;
  ...
  pert_resp.formant_out.dat{ipert_type} = pert_resp.formant_out.dat{ipert_type} - baseline_repmat;
...
subplot(211)
plot(parsed_frame_taxis,pert_resp.pitch_in.mean(3,:),'k')
hold on
plot(parsed_frame_taxis,pert_resp.pitch_in.mean(2,:),'r')
plot(parsed_frame_taxis,pert_resp.pitch_in.mean(1,:),'b')
subplot(212)
plot(parsed_frame_taxis,pert_resp.pitch_in.mean(3,:),'k')
hold on
plot(parsed_frame_taxis,pert_resp.pitch_in.mean(2,:),'r')
...
if (yes_overwrite >= 2) || ~exist('tlims4comp.mat','file')
  formant_make_tlims4comp(pert_resp,colors,parse_fig_pos);
end
load('tlims4comp');

if (yes_overwrite >= 1) || ~exist('pert_resp.mat','file')
  for ipert_type = 1:npert_types
    pert_resp.mean_signed_response{ipert_type} = mean(pert_resp.pitch_in.dat{ipert_type}(:,idxes4comp),2);
    pert_resp.comp{ipert_type} = -(pert_resp.mean_signed_response{ipert_type})/pert_resp.pert_types(ipert_type);
  end
  tflat_rest_of_resp = [];
  yes_peakpick = 0;
  xcspec = formant_get_pert_resp_xcspec(pert_resp,[],tlims4baseline,[],1,tflat_rest_of_resp,yes_peakpick);
  npert_types = pert_resp.npert_types;
  for ipert_type = 1:npert_types
...
for ipert_type = 1:(npert_types+1)
  baseline_repmat = repmat(pert_resp.baseline,[pert_resp.n_good_trials(ipert_type) 1]);
  pert_resp.formant_in.t(ipert_type,:) = ttest(pert_resp.formant_in.dat{ipert_type});
  pert_resp.formant_in.median(ipert_type,:) = median(pert_resp.formant_in.dat{ipert_type},1);
  pert_resp.formant_out.median(ipert_type,:) = median(pert_resp.formant_out.dat{ipert_type},1);
end
...
for iipert_type = 1:npert_types
  ...
  if yes_median4mean
    m = pert_resp.formant_in.median(ipert_type,:);
  else
    m = pert_resp.formant_in.mean(ipert_type,:);
  end

  se = pert_resp.formant_in.stde(ipert_type,:);
  ...
  if yes_median4mean
    m = pert_resp.formant_out.median(ipert_type,:);
  else
    m = pert_resp.formant_out.mean(ipert_type,:);
  end
  ...
  se = pert_resp.formant_out.stde(ipert_type,:);
  hp = plot(parsed_frame_taxis,m,'b'); set(hp,'LineWidth',3);
  hp = plot(parsed_frame_taxis,m-se,'b'); set(hp,'LineWidth',1);
  hp = plot(parsed_frame_taxis,m+se,'b'); set(hp,'LineWidth',1);
  ...
  plot(parsed_frame_taxis,pert_resp.formant_in.dat{ipert_type}');
...
plotname = sprintf('subj%d_formant_comp_hist',isubj);
...
if yes_plot_trials2print
  ...
  for iipert_type = npert_types:(-1):1
    ...
    formant_in4pert =                formant_in(good_trials4pert,:);
    formant_out4pert =              formant_out(good_trials4pert,:);
    formant_perts4pert =            formant_pert(good_trials4pert,:);
    ...
    ii4perttrial = 0;
     ...
     for iiblock4pert = 1:nblocks4pert
      ...
      for itr = 1:ntrials_in_block
        ...
        formant_in4trial =   formant_in4pert(ii4perttrial,:);
        formant_out4trial = formant_out4pert(ii4perttrial,:);
        pert_formant_in4trial = pert_resp.formant_in.dat{iipert_type}(ii4perttrial,:);
        pert_formant_out4trial = pert_resp.formant_out.dat{iipert_type}(ii4perttrial,:);
        formant_pert4trial = formant_perts4pert(ii4perttrial,i_winstart:i_winstop);
  ...
  if yes_show_wave_out, isubfig = isubfig + 1; hax(isubfig) = my_subplot(nsubfigs,1,isubfig,fig_bord,subplot_topbord,subplot_rightbord);
        ...
        if yes_show_hz, isubfig = isubfig + 1; hax(isubfig) = my_subplot(nsubfigs,1,isubfig,fig_bord,subplot_topbord,subplot_rightbord);
          plot(pert_taxis_frame,squeeze(formant_out4trial(i_winstart:i_winstop)),'b'); hold on
          plot(pert_taxis_frame,squeeze(formant_in4trial(i_winstart:i_winstop)),'r');  hold off
          set(gca,'XTickLabel',[]);
          ylabel('pitch (Hz)');
         ...
         if yes_show_cents, isubfig = isubfig + 1; hax(isubfig) = my_subplot(nsubfigs,1,isubfig,fig_bord,subplot_topbord,subplot_rightbord);
          hpl = plot(pert_taxis_frame,pert_formant_cents_out4trial,'b'); set(hpl,'LineWidth',3); hold on
          hpl = plot(pert_taxis_frame,pert_formant_cents_in4trial,'r'); set(hpl,'LineWidth',3); 
          if yes_show_pert, hpl = plot(pert_taxis_frame,formant_pert4trial,'g'); set(hpl,'LineWidth',3); end
          hold off
          xlabel('time (sec)');
          ylabel('pitch (cents)');
          a = axis; axis([pert_taxis_frame(1) pert_taxis_frame(end) a(3:4)]);
...
if yes_plot_corrcomp
  ...
  if yes_plot_formant_comp_corr
    formant_trial_dat = mean([(pert_resp.pitch_in.dat{1}(:,i_itvl4corr(1):i_itvl4corr(2))); ...
                        (pert_resp.pitch_in.dat{2}(:,i_itvl4corr(1):i_itvl4corr(2)))],2);
    comp_trial_dat = [(pert_resp.comp{1});(pert_resp.comp{2})];
    [b,duh,duh1,duh2,stats] = regress(comp_trial_dat,[ones(size(formant_trial_dat)) formant_trial_dat]);
    r_formant_comp = sqrt(stats(1));
    p_formant_comp = stats(3);
    axes(corrcomp_hax(2));
    plot(formant_trial_dat,comp_trial_dat,'*');
    hrl = refline(b(2),b(1));
    set(hrl,'Color','r');
    set(hrl,'LineWidth',2);
    xlabel('pitch (Hz)');
    ylabel('comp');
    title(sprintf('subj(%d), r(%.2f), p(%.5f)',isubj,r_formant_comp,p_formant_comp));
    axes(trialnum_hax(2));
    plot([pert_resp.good_trials{1} pert_resp.good_trials{2}],formant_trial_dat,'*');
    xlabel('trial #');
    ylabel('pitch (Hz)');
    title(sprintf('subj(%d)',isubj));
  end
